...
async function getWeightedAverageRatingsPerWine() {
  try {
    ...
    const weightedAvgRatingsPerWine = numRatingsPerWine.map(numRating => {
      const avgRating = avgRatingsPerWine.find(avgRating => avgRating.IDVinho === numRating.IDVinho);
      const weightedAvgRating = 0.4 * numRating.num_ratings + 0.6 * avgRating.avg_rating;
      return {
...
async function updateWineWeightedAverageRatings() {
  try {
    ...
    for (const weightedAvgRating of weightedAvgRatingsPerWine) {
      ...
      console.log(`Updated ${rows.affectedRows} rows for wine ID ${weightedAvgRating.IDVinho} and rating is${weightedAvgRating.weighted_avg_rating}`);
    }
    connection.release();
